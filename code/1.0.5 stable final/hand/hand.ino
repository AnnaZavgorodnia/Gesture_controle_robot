...
//
//byte SWITCH_OFF_BUTTON = 2;
//
//pinMode(SWITCH_OFF_BUTTON, OUTPUT);
...
// axelerometer + gyroscope MPU-6050
...
if (angle_x < -20 || angle_x > 20) {
  stay = false;
  if (angle_x < 0) { // forward_speed
    dir = DIR_FORWARD;
    robo_speed = map(-1 * angle_x, 0, 90, 50, 250);
    constrain(angle_x, 50, 250);
  } else { // behind_speed
    dir = DIR_BACKWARD;
    robo_speed = map(angle_x, 0, 90, 50, 250);
    constrain(angle_x, 50, 250);
  }
}
if (angle_y < -20 || angle_y > 20) {
  stay = false;
  if (angle_y < 0) { // left_speed
    dir = DIR_LEFT;
    robo_speed = map(-1 * angle_x, 0, 90, 50, 250);
    constrain(angle_y, 50, 250);
  } else { // right_speed
    dir = DIR_RIGHT;
    robo_speed = map(angle_x, 0, 90, 50, 250);
    constrain(angle_y, 50, 250);
  ...
  if (stay == false) {
    sendDirection(dir, 250);
  } else {
    sendDirection(DIR_NONE, 0);
  }
}
...
void sendDirection(const byte dir, const byte robotSpeed) {
//  byte massage[2];
//  massage[0] = dir;
//  massage[1] = robotSpeed;

  int massage[] = {0, 0, 0, 0};
  massage[0] = 1;
  massage[1] = 0;
  massage[2] = dir;
  massage[3] = robotSpeed;
  
  radio.write(&massage, sizeof(massage));
...
//  radio.write(&massage, sizeof(massage));
  Serial.print(massage[0]);
  Serial.print(" | DIR = ");
  Serial.println(massage[2]);
//  printDirection(dir);
//  Serial.println(robotSpeed);
}

// axelerometer + gyroscope MPU-6050 dunctions 
// --------------------------------------------------------
// MPU6050_read
//
